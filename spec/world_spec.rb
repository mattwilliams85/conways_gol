describe 'World' do
  ...
  it 'births and kills the appropriate cells' do
    ...
    expect(test_world.neighbor_check(test_world.grid,1,1)).to eq "birth"
    expect(test_world.neighbor_check(test_world.grid,0,1)).to eq "remain"
    expect(test_world.neighbor_check(test_world.grid,4,4)).to eq "remain"
    expect(test_world.neighbor_check(test_world.grid,2,2)).to eq "kill"
  end
